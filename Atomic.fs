...
type AtomicBool(initialValue: bool) =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member __.Swap(nval: bool) : bool =
        Interlocked.Exchange(&value, (if nval then 1 else 0)) = 1
...
type AtomicInt(initialValue: int) =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member __.CompareAndSwap(compared: int, nval: int) : bool =
        Interlocked.CompareExchange(&value, nval, compared) = compared
...
type AtomicInt64(initialValue: int64) =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member __.CompareAndSwap(compared: int64, nval: int64) : bool =
        Interlocked.CompareExchange(&value, nval, compared) = compared
...
type AtomicFloat(initialValue: float) =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member __.CompareAndSwap(compared: float, nval: float) : bool =
        Interlocked.CompareExchange(&value, nval, compared) = compared
...
type AtomicFloat32(initialValue: float32) =
    ...
    [<MethodImpl(MethodImplOptions.AggressiveInlining)>]
    member __.CompareAndSwap(compared: float32, nval: float32) : bool =
        Interlocked.CompareExchange(&value, nval, compared) = compared
...
module Atomic =
    ...
    let inline cas (expected: 'a) (nval: 'a) (atom: #IAtomic<'a>) = atom.CompareAndSwap(expected, nval)
