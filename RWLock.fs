...
type private LockState =
    ...
    member this.AdjustRead(id: LockId, delta: int) =
        ...
        | Read map ->
            ...
            if modified > 0 then
                Read(Map.add id modified map)
            elif modified = 0 then
                Read(Map.remove id map)
            else
                failwithf "Bug (read lock): task %i is releasing the lock it doesn't hold." id
        | Write(writes, reads, ownerId) when ownerId = id ->
            ...
            if modified >= 0 then
                ...
                failwithf "Bug (write lock): task %i is releasing the lock it doesn't hold." id
...
type private State<'t> =
    ...
    static member Create(initialValue: 't) =
        { value = initialValue
...
and [<Sealed; NoComparison; NoEquality>] RWLock<'a>(initialValue: 'a) =
    static let cancelWrite: Action<obj, CancellationToken> =
        Action<_, _>(fun o ct ->
            let promise = o :?> TaskCompletionSource<WriteLockAcquisition<'a>>
            promise.TrySetCanceled(ct) |> ignore)

    static let cancelRead: Action<obj, CancellationToken> =
        Action<_, _>(fun o ct ->
    ...
    member internal this.GetValue() =
        ...
        if obj.ReferenceEquals(null, s) then
            LockUtils.raiseDisposedException ()
        else
            s.value
    ...
    member internal this.SetValue(value: 'a) =
        let rec loop state value =
            ...
            if obj.ReferenceEquals(null, old) then
                LockUtils.raiseDisposedException ()
    ...
    member internal this.AcquireRead
        (
            id: LockId,
            cancellationToken: CancellationToken
        ) : TaskCompletionSource<_> voption =
    ...
    member internal this.AcquireWrite
        (
            id: LockId,
            cancellationToken: CancellationToken
        ) : TaskCompletionSource<_> voption =
        ...
        let rec attempt (state: AtomicRef<_>) (id: LockId) =
            ...
            if updated.IsReadLocked then
                ...
                if pollNonCancelled (&writers, &pending) then
                    ...
                    // failsafe - try complete potential readers if possible
                ...
                if not (obj.ReferenceEquals(old.stm, updated)) then
                    ...
                    then
                        if pending.awaiter.TrySetResult(new ReadLockAcquisition<'a>(this, pending.id)) then
                            ...
                            |> Atomic.update (fun s ->
                                { s with
                                    stm = s.stm.AdjustRead(pending.id, -1) })
                            |> ignore
        ...
        let rec attempt (state: AtomicRef<_>) (id: LockId) =
            ...
            let mutable updated =
                ...
                | Write(w, r, ownerId) when ownerId = id ->
                    ...
                    if writes < 0 then
                        failwithf "Bug: task %i released write lock that was not held." id
                    elif writes = 0 then
    ...
    member this.Write(?cancellationToken: CancellationToken) : ValueTask<WriteLockAcquisition<'a>> =
        ...
        match this.AcquireWrite(id, cancellationToken) with
        | ValueNone -> ValueTask<WriteLockAcquisition<'a>>(new WriteLockAcquisition<'a>(this, id))
    ...
    member this.Dispose() =
        ...
        for pending in old.readerQ do
            pending.Dispose()

        for pending in old.writerQ do
            pending.Dispose()
